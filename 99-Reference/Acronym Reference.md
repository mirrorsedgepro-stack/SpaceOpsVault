---
title: Acronym Reference
classification: UNCLASSIFIED // FOUO
tags:
  - reference
  - acronyms
---

# Acronym Reference

> [!abstract] Quick Summary
> A glossary of 80+ acronyms used throughout this vault — the essential cheat sheet for newcomers encountering military space jargon for the first time.

| Acronym | Meaning |
| --- | --- |
| AEHF | Advanced Extremely High Frequency (SATCOM) |
| AMTI | Air Moving Target Indicator |
| ATLAS | Advanced Tracking and Launch Analysis System |
| AUSSpOC | Australian Space Operations Centre |
| BDA | Battle Damage Assessment |
| BLOS | Beyond Line of Sight |
| CEC | Cooperative Engagement Capability |
| CDL | Common Data Link |
| CFC | Combat Forces Command |
| CSpOC | Combined Space Operations Center |
| DA-ASAT | Direct-Ascent Anti-Satellite |
| DPI | Desired Point of Impact |
| DRFM | Digital Radio Frequency Memory |
| EA | Electronic Attack |
| ELINT | Electronic Intelligence |
| EMCON | Emissions Control |
| EMSO | Electromagnetic Spectrum Operations |
| EOR | Engage-on-Remote |
| EOB | Electronic Order of Battle |
| EP | Electronic Protection |
| EPS | Evolved Polar System (AEHF replacement) |
| ES/ESM | Electronic Support / Measures |
| F2T2EA | Find, Fix, Track, Target, Engage, Assess |
| FISINT | Foreign Instrumentation Signals Intelligence |
| GEO | Geosynchronous Earth Orbit (~35,786 km) |
| GEODSS | Ground-based Electro-Optical Deep Space Surveillance |
| GMTI | Ground Moving Target Indicator |
| GNSS | Global Navigation Satellite System |
| GSSAP | Geosynchronous Space Situational Awareness Program |
| HEO | Highly Elliptical Orbit |
| INS | Inertial Navigation System |
| ISR | Intelligence, Surveillance and Reconnaissance |
| JADC2 | Joint All-Domain Command and Control |
| JREAP | Joint Range Extension Applications Protocol |
| JSTARS | Joint Surveillance and Target Attack Radar System (E-8C, retired 2023) |
| LEO | Low Earth Orbit (160–2,000 km) |
| LPI/LPD | Low Probability of Intercept / Detection |
| LRDR | Long Range Discrimination Radar |
| MADL | Multifunction Advanced Data Link |
| MEO | Medium Earth Orbit (2,000–35,786 km) |
| MUOS | Mobile User Objective System |
| NB | Narrowband |
| NIFC-CA | Naval Integrated Fire Control – Counter Air |
| NMA | Navigation Message Authentication |
| NRO | National Reconnaissance Office |
| OPIR | Overhead Persistent Infrared |
| PNT | Positioning, Navigation and Timing |
| PRS | Public Regulated Service (Galileo) |
| PWSA | Proliferated Warfighter Space Architecture (SDA constellation) |
| RPO | Rendezvous and Proximity Operations |
| S4S | US Space Forces-Space |
| SATCOM | Satellite Communications |
| SBIRS | Space-Based Infrared System |
| SDA | Space Domain Awareness |
| SMC | Short Message Communication (BeiDou tactical datalink) |
| SSA | Space Situational Awareness (predecessor to SDA) |
| SSN | Space Surveillance Network |
| SST | Space Surveillance Telescope |
| STARCOM | Space Training and Readiness Command |
| SWORD | Space Warfighter Operational Readiness Domain |
| TDOA | Time Difference of Arrival (geolocation method) |
| TERCOM | Terrain Contour Matching |
| TLE | Two-Line Element (orbital data format) |
| TTNT | Tactical Targeting Network Technology |
| UHF | Ultra High Frequency |
| USSF | United States Space Force |
| USSPACECOM | United States Space Command |
| WB | Wideband |
| WGS | Wideband Global SATCOM |

> [!tip] Hot Tip
> When in doubt about an acronym, check here first — but be aware that some acronyms (e.g., SDA, CSpO) have changed meaning over time as USSF doctrine evolved. If an acronym doesn't match the context, check the full note for the current definition.

> [!warning]- Constraints, Limitations and Assumptions
> **Constraints:** None.
>
> **Limitations:** Acronyms in this vault reflect US/ADF usage — some terms have different meanings in other allied nations' lexicons (e.g., UK, Canada).
>
> **Assumptions:** Assumes acronyms remain stable — USSF has introduced new terminology since its 2019 establishment and will continue to do so.

---

**Related:** [[Space Operations Reference — Map of Content]]
